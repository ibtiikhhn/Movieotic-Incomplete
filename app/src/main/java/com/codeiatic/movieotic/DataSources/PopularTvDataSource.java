...
import com.codeiatic.movieotic.NetworkUtils.PopularTV;
...
public class PopularTvDataSource extends PageKeyedDataSource<Integer, Result> {
    ...
    public PopularTvDataSource() {
        ...
        popularTV = retrofit.create(PopularTV.class);
    }
    ...
    public void loadInitial(@NonNull PageKeyedDataSource.LoadInitialParams<Integer> params, @NonNull final PageKeyedDataSource.LoadInitialCallback<Integer, Result> callback) {
        popularTV.getTVs("381054badc0dabe1a386b89e1d169c2d", String.valueOf(firstPage)).enqueue(new Callback<Example>() {
            @Override
    ...
    @Override
    public void loadBefore(@NonNull final PageKeyedDataSource.LoadParams<Integer> params, @NonNull final LoadCallback<Integer, Result> callback) {
        popularTV.getTVs("381054badc0dabe1a386b89e1d169c2d", String.valueOf(params.key)).enqueue(new Callback<Example>() {
            @Override
            public void onResponse(Call<Example> call, Response<Example> response) {
                if (response.body() != null) {
